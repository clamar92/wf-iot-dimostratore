# app_step2.py — DT Request Demo (no sidebar) — trust evaluation with popovers (no X)
import numpy as np
...
PAGE_TAG = "step2"  # prefisso per le chiavi della pagina

st.set_page_config(page_title="DT Request Demo — Step 2", layout="wide")
...
class Scene:
    ...
    comp_cap: dict = field(default_factory=dict)  # QoS: computation capabilities per DT
    time_resp: dict = field(default_factory=dict)  # QoS: time to response per DT (s)
    agg_direct: dict = field(default_factory=dict)  # cache: vettori direct per provider
    agg_indirect: dict = field(default_factory=dict)  # cache: vettori indirect per provider
    last_plot_key: tuple | None = None           # evita ridisegno inutile

SCENE_KEY = "scene_step2"
...
def provider_reading(
    service: str, provider_id: int, bad_by_service: dict[str, set[int]], rng: np.random.Generator
) -> float:
    """
    Valori per servizio:
...
def ensure_network_ready():
    if not S.created:
        ...
        # QoS init: assegna a ogni DT i valori iniziali
        rng_qos = np.random.default_rng(S.seed + 424242)
        S.comp_cap = {i: float(rng_qos.uniform(0.5, 1.0)) for i in range(S.n)}
        S.time_resp = {i: float(rng_qos.uniform(0.2, 0.6)) for i in range(S.n)}
...
def draw_graph_if_needed(force=False):
    ...
    if force or (S.last_plot_key != key_state):
        ...
        graph_placeholder.plotly_chart(
            fig, use_container_width=True, key=GRAPH_KEY, config={"displayModeBar": False}
        )
    ...
    if S.providers:
        ...
        if S.selected is not None:
            ...
            if key not in S.readings:
                ...
                S.readings[key] = provider_reading(
                    S.service, S.selected, S.bad_by_service, rng_read
                )
            val = S.readings[key]
